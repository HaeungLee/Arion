...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // 음성 녹음 + STT - API 명세서와 일치하도록 수정
    if (pathname === '/api/record-and-transcribe' && method === 'POST') {
    ...
    // TTS - API 명세서와 일치하도록 수정
    if (pathname === '/api/text-to-speech' && method === 'POST') {
      const body = await parseRequestBody(req);
    ...
    // 통합 음성 명령 - API 명세서와 일치하도록 수정
    if (pathname === '/api/voice-command' && method === 'POST') {
      const body = await parseRequestBody(req);
    ...
    // 녹음 시간 프리셋 - API 명세서와 일치하도록 수정
    if (pathname === '/api/duration-presets' && method === 'GET') {
    ...
    // 서비스 정보 API 추가
    if (pathname === '/api/service-info' && method === 'GET') {
      const result = await proxyToFastAPI('/api/service-info', 'GET');
      sendJSONResponse(res, result.statusCode || 200, result.data);
      return;
    }
    ...
    if (pathname === '/api/voices' && method === 'GET') {
      const result = await proxyToFastAPI('/api/voices', 'GET');
      sendJSONResponse(res, result.statusCode || 200, result.data);
      return;
    }
...
server.listen(PORT, () => {
  ...
  console.log('  POST /api/record-and-transcribe');
  console.log('  POST /api/text-to-speech');
  console.log('  POST /api/voice-command');
  console.log('  GET  /api/duration-presets');
