...
const main = async (): Promise<void> => {
  ...
  await server.open(Number(SGlobal.env.PORT), async (acceptor) => {
    ...
    const agent: Agentica<"chatgpt"> = new Agentica({
      model: "chatgpt",
      vendor: vendorConfig as any, // 필요한 경우에만 type assertion 사용
      config: {
        locale: "ko-KR",
        executor: {
...
🎯 응답 규칙 🎯
- 모든 응답은 반드시 한국어로만 작성
- 사용자가 "상세히" 또는 "자세히"를 요청하면 구체적이고 상세한 정보 제공
- 일반적인 경우에는 간결하고 명확하게 응답
- Function call 결과는 한국어로만 설명

응답 스타일:
- 간결 모드: 핵심만 간단히 (예: "메일 전송 완료")
- 상세 모드: 구체적 정보 포함 (예: "Gmail에서 총 15개의 새로운 메일을 확인했습니다. 이 중 5개는 업무 관련, 3개는 개인, 7개는 프로모션 메일입니다.")`,
          
          execute: () => `한국어로만 응답하세요. 사용자의 요청에 따라 간결하거나 상세하게 설명하세요.`,
          
          describe: () => `함수 실행 결과를 한국어로 명확하게 설명하세요. 사용자가 이해하기 쉽게 설명해주세요.`,
          
          initialize: () => `한국어로만 응답하세요. 사용자를 도울 준비가 되었습니다.`,
          
          select: () => `한국어로만 응답하세요. 선택한 이유를 간단히 설명해주세요.`
        }
      },
      controllers: [
        /*
