    ...
    const checkAuthStatus = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          dispatch({ type: 'SET_USER', payload: user });
          console.log('✅ 사용자 인증 상태: 인증됨', user);
        }
      } catch (error) {
        // Spring Security 서버 연결 에러 vs 인증 에러 구분
        if (error instanceof Error && (
          error.message.includes('Failed to fetch') || 
          error.message.includes('ERR_CONNECTION_REFUSED')
        )) {
          // 개발 환경에서는 서버 연결 실패를 무시
          if (import.meta.env.DEV) {
            console.warn('⚠️ Spring Security 서버 (8080) 연결 실패 - 개발 환경에서는 정상');
            dispatch({ type: 'SET_ERROR', payload: null });
          } else {
            console.error('❌ 인증 서버 연결 실패:', error.message);
            dispatch({ type: 'SET_ERROR', payload: '인증 서버에 연결할 수 없습니다.' });
          }
        } else {
          console.log('🔐 사용자 인증 상태: 미인증');
          dispatch({ type: 'SET_ERROR', payload: null });
        }
      }
    };
