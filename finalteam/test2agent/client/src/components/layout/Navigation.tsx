  ...
  return (
    <>
      ...
      <div className="relative">
        <nav 
          className={`fixed top-16 left-0 h-[calc(100vh-4rem)] bg-gray-900/95 backdrop-blur-sm text-white overflow-y-auto border-r border-gray-700/50 z-30 transform transition-all duration-300 ease-in-out
                    ${state.isOpen ? 'w-64 translate-x-0' : 'w-16 translate-x-0'}`}
        >
          <div className="p-4 flex flex-col h-full">
            <div className="space-y-6">
              {/* 로고/제목 및 토글 버튼 */}
              <div className={`flex items-center ${state.isOpen ? 'justify-between' : 'justify-center'}`}>
                {state.isOpen && (
                  <div 
                ...
                <button 
                  ...
                  className={`p-2 rounded-lg hover:bg-gray-700/50 transition-all duration-200 text-gray-400 hover:text-white ${
                    !state.isOpen ? 'absolute top-4 right-2' : ''
                  }`}
